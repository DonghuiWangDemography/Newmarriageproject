...
global dv1 "edu_fm lassetp10                 sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize Lmigrant i.region i.spell"
global dv2 "edu_fm lassetp10 Leduy Llincome  sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize Lmigrant i.region i.spell"

// global dv1 "edu_fm lassetp10                 i.spell"
// global dv2 "edu_fm lassetp10 Leduy Llincome  i.spell"


// global dv1 "edu_fm  lwealthp                 sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize i.region i.spell"
// global dv2 "edu_fm  lwealthp Leduy Llincome  sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize i.region i.spell"

global ctr "sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize i.region i.spell"


*sample 
global uf "male==0 & urbanhukou10==1"
global um "male==1 & urbanhukou10==1"
global rf "male==0 & urbanhukou10==0"
global rm "male==1 & urbanhukou10==0"
global urban "urbanhukou10==1"
global rural "urbanhukou10==0"

*calculate quntiles of wealth
local sample "uf um rf rm urban rural"

foreach x of local sample {
sum lassetp10 if $`x' , detail
return list 
scalar `x'_p5=r(p5)
scalar `x'_p10=r(p10)
scalar `x'_p25=r(p25)
...
mat `x'=`x'_p5\ `x'_p10 \ `x'_p25 \ `x'_p50 \ `x'_p75 \ `x'_p90 \ `x'_p95
...
*urban 
	logit married $`x'  if male==0 & urbanhukou10==1 , or vce(robust)	
	margins, at(lassetp10=(6.21 9.61 11.48 12.29 13.16 14.16 14.46)) post 
	est store uf_`x'
	
	logit married $`x'  if male==0 & urbanhukou10==1 , or vce(robust)	
	margins, at(lassetp10=(8.29 10.12 11.43 12.46 13.34 14.11 14.41)) post 
	est store um_`x'
	
*rural
	logit married $`x'  if male==1 & urbanhukou10==0 , or vce(robust)	
	margins, at(lassetp10=(9.97 10.45  11.06 11.69 12.27 12.85 13.22)) post 
	est store rm_`x'
	
	logit married $`x'  if male==0 & urbanhukou10==0 , or vce(robust)	
	margins, at(lassetp10=(9.97 10.45  11.06 11.69 12.27 12.85 13.22)) post 
	est store rf_`x'
...
coefplot uf_dv2 um_dv2  , at title("Urban Sample")  xtitle("Family wealth quantile") sort recast(line) lwidth(*2) 

coefplot rm_dv2 rf_dv2  , at title("rural Sample")  xtitle("Family wealth quantile") sort recast(line) lwidth(*2) 


graph save Graph "$graphs\uf", replace 

*urban male 	
mat list um	
local dv "dv1 dv2"
foreach x of local dv{
	logit married $`x'  if male==1 & urbanhukou10==1 , or vce(robust)	
	margins, at(lassetp10=(8.29 10.12 11.43 12.46 13.34 14.11 14.41)) post 
	est store `x'
// 	margins, dydx(edu_fm lassetp10)
// 	mat c=r(table)
...
coefplot dv2 dv1  , at title("Urban Male")  recast(line)  sort  lwidth(*1)  

graph save Graph "$graphs\um", replace 
...
foreach x of local dv{
	logit married $`x'  if male==1 & urbanhukou10==0 , or vce(robust)	
	margins, at(lassetp10=(9.82 10.39 11.03 11.73 12.36 12.94 13.30)) post 
	est store `x'
// 	margins, dydx(edu_fm lassetp10)
// 	mat c=r(table)
// 	mat ci_uf_`x'=c["ll".."ul",1..2]
...
coefplot dv2 dv1  , at title("Rural Male")   sort recast(line) lwidth(*1)
graph save Graph "$graphs\rm", replace 
...
foreach x of local dv{
	logit married $`x'  if male==1 & urbanhukou10==0 , or vce(robust)	
	margins, at(lassetp10=(10.01 10.54 11.10 11.68 12.26 12.93 13.42)) post 
	est store `x'
// 	margins, dydx(edu_fm lassetp10)
// 	mat c=r(table)
// 	mat ci_uf_`x'=c["ll".."ul",1..2]
...
coefplot dv2 dv1  , at title("Rural Female")  noci sort recast(line) lwidth(*1)
graph save Graph "$graphs\rf", replace 

grc1leg   "$graphs\rm" "$graphs\rf" "$graphs\um" "$graphs\uf" , legendfrom("$graphs\uf" ) ycommon 
...
// coefplot  (matrix(b_uf_dv1) ,ci(ci_uf_dv1) label(female(total)) )  ///
...
local gr "uf um rf rm"
foreach x of local gr {
mat colnames b_`x'_dv1=parental_education family_asset
mat colnames b_`x'_dv2=parental_education family_asset

mat colnames ci_`x'_dv1=parental_education family_asset
mat colnames ci_`x'_dv2=parental_education family_asset
}


*urban 
coefplot  (matrix(b_uf_dv1),ci(ci_uf_dv1) label(female(total)))  ///
		  (matrix(b_uf_dv2),ci(ci_uf_dv2) label(female(direct)))  ///
		  (matrix(b_um_dv1),ci(ci_um_dv1) label(male(total)))  ///
		  (matrix(b_um_dv2),ci(ci_um_dv2) label(male(direct))) , ///
		   yline(0) msymbol(S)  vertical   title("Average Marginal Effects (Urban)")
graph save Graph "${graphs}\ame_urbanmarriage_$date.gph",replace


*rural 
coefplot  (matrix(b_rf_dv1),ci(ci_rf_dv1) label(female(total))) )  ///
		  (matrix(b_rf_dv2),ci(ci_rf_dv2) label(female(direct)) )  ///
		  (matrix(b_rm_dv1),ci(ci_rm_dv1) label(male(total)))  ///
		  (matrix(b_rm_dv2),ci(ci_rm_dv2) label(male(direct))) , ///
...
*by gender 
coefplot  (matrix(b_uf_dv1),ci(ci_uf_dv1) label(urban female(total)))  ///
		  (matrix(b_uf_dv2),ci(ci_uf_dv2) label(urban female(direct)))  ///
		  (matrix(b_rf_dv1),ci(ci_rf_dv1) label(rural female(total)))  ///
		  (matrix(b_rf_dv2),ci(ci_rf_dv2) label(rural female(direct))) , ///
		   yline(0) msymbol(S)  vertical   title("Average Marginal Effects (Female)")
graph save Graph "${graphs}\ame_urbanmarriage_$date.gph",replace

coefplot  (matrix(b_um_dv1),ci(ci_um_dv1) label(urban male(total)))  ///
		  (matrix(b_um_dv2),ci(ci_um_dv2) label(urban male(direct)))  ///
		  (matrix(b_rm_dv1),ci(ci_rm_dv1) label(rural male(total)))  ///
		  (matrix(b_rm_dv2),ci(ci_rm_dv2) label(rural male(direct))) , ///
		   yline(0) msymbol(S)  vertical   title("Average Marginal Effects (Male)")
...
local gr "uf um rf rm"
foreach x of local gr {
mat ed_`x'_dv1 = b_`x'_dv1[1,1] \ ci_`x'_dv1[1..2,1]
mat ed_`x'_dv2 = b_`x'_dv2[1,1] \ ci_`x'_dv2[1..2,1]


mat wt_`x'_dv1 = b_`x'_dv1[1,2] \ ci_`x'_dv1[1..2,2]
mat wt_`x'_dv2 = b_`x'_dv1[1,2] \ ci_`x'_dv2[1..2,2]
}

set scheme _grstyle_ 


coefplot (matrix(wt_uf_dv1[1]), ci((wt_uf_dv1[2] wt_uf_dv1[3])) label(urban female(total)))   ///
		 (matrix(wt_uf_dv2[1]), ci((wt_uf_dv2[2] wt_uf_dv2[3])) label(urban female(direct)))   ///
		 (matrix(wt_um_dv1[1]), ci((wt_um_dv1[2] wt_um_dv1[3])) label(urban male(total)))      ///
		 (matrix(wt_um_dv2[1]), ci((wt_um_dv2[2] wt_um_dv2[3])) label(urban male(direct)))    ///
		 (matrix(wt_rf_dv1[1]), ci((wt_rf_dv1[2] wt_rf_dv1[3])) label (rural female(total)))  ///
		 (matrix(wt_rf_dv2[1]), ci((wt_rf_dv2[2] wt_rf_dv2[3])) label (rural female(direct)))   ///
		 (matrix(wt_rm_dv1[1]), ci((wt_rm_dv1[2] wt_rm_dv1[3])) label (rural male(total)))     ///
		 (matrix(wt_rm_dv2[1]), ci((wt_rm_dv2[2] wt_rm_dv2[3])) label (rural male(direct))) ,  ///
		 yline(0) msymbol(S) vertical title("Average Marginal Effects (Household wealth)") legend(off) ///
		 xlabel(1 "urban female(total)" 2 "urban female(direct)"  3"urban male(total)"  4 "urban male(direct)" ///
		        5 "rural female(total)" 6 "rural female(direct)"  7 "rural male(total)" 8 "rural male(direct)" , angle(45))
...
coefplot (matrix(wt_uf_dv1[1]), ci((wt_uf_dv1[2] wt_uf_dv1[3])) label(urban female(total)))   ///
		 ...
		 (matrix(wt_rm_dv1[1]), ci((wt_rm_dv1[2] wt_rm_dv1[3])) label (rural male(total)))     ///
		 (matrix(wt_rm_dv2[1]), ci((wt_rm_dv2[2] wt_rm_dv2[3])) label (rural male(direct))) ,  ///
		 yline(0) msymbol(S) vertical title("Average Marginal Effects (Household wealth)") xlabel(, nolabels)

				

coefplot (matrix(ed_uf_dv1[1]), ci((ed_uf_dv1[2] ed_uf_dv1[3])) label(urban female(total)))   ///
		 (matrix(ed_uf_dv2[1]), ci((ed_uf_dv2[2] ed_uf_dv2[3])) label(urban female(direct)))   ///
		 (matrix(ed_um_dv1[1]), ci((ed_um_dv1[2] ed_um_dv1[3])) label(urban male(total)))      ///
		 (matrix(ed_um_dv2[1]), ci((ed_um_dv2[2] ed_um_dv2[3])) label(urban male(direct)))    ///
		 (matrix(ed_rf_dv1[1]), ci((ed_rf_dv1[2] ed_rf_dv1[3])) label (rural female(total)))  ///
		 (matrix(ed_rf_dv2[1]), ci((ed_rf_dv2[2] ed_rf_dv2[3])) label (rural female(direct)))   ///
		 (matrix(ed_rm_dv1[1]), ci((ed_rm_dv1[2] ed_rm_dv1[3])) label (rural male(total)))   ///
		 (matrix(ed_rm_dv2[1]), ci((ed_rm_dv2[2] ed_rm_dv2[3])) label (rural male(direct))) ,  ///
		 yline(0) msymbol(S)  vertical  title("Average Marginal Effects (Parental Education)") xlabel(, nolabels) 
...
esttab female_rural male_rural female_urban male_urban  using "$tables\khbmarriage_$date.rtf",  ///
       b(%9.3f)  se(%9.3f) mtitles replace 
log close 
...
*separate effet of edu and wealth 
...
*========heterogenous effect by age=================
global dv3 "edu_fm lassetp10 Leduy Llincome  sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize Lmigrant i.region i.spell"
...
*local iv "edu_fm lassetp10 Leduy Llincome  sp1 sp2 sp3 han Llivepa Lfarmhh Lnonagbushh  dagepa Lfamilysize hasbro ftree10 grave10 i.region i.spell"
*local iv "edu_fm Llassetp   Leduy Llincome  sp1 sp2 sp3 han Llivepa Lfarmhh Lnonagbushh  dagepa Lfamilysize hasbro ftree10 grave10 i.region i.spell"
*local iv "edu_fm Lown_p    Leduy Llincome   sp1 sp2 sp3 han Llivepa Lfarmhh Lnonagbushh  dagepa Lfamilysize hasbro ftree10 grave10 i.region i.spell"
*local iv "edu_fm Lhouse_owned    Leduy Llincome  sp1 sp2 sp3 han Llivepa Lfarmhh Lnonagbushh  dagepa Lfamilysize hasbro ftree10 grave10 i.region i.spell"  // strong predictor of marriage transition 
...
global iv "edu_fm  lassetp10 Leduy Llincome i.Loccup  sp1 sp2 sp3 han  Lfarmhh Lnonagbushh  dagepa Lfamilysize hasbro i.region "  // strong predictor of marriage transition 
...
esttab male_rural_livepa female_rural_livepa male_urban_livepa female_urban_livepa using  "$tables\livingarragement.rtf" ,    ///
       nonumbers mtitles  eform  ci(%9.2f) noomitted la replace 



*=====conditional on marriage, interaction btw children and living arragement========
g 		nchildren=nchildren_12hh  if  marstat12==1 
replace nchildren=nchildren_14hh  if  marstat14==1   & marstat12==0
replace nchildren=nchildren_16hh  if  marstat16==1   & marstat14==0

g children=(nchildren > 0 & nchildren< .) 

*global iv "edu_fm Lhouse_sqr  Leduy Llincome i.Loccup  sp1 sp2 sp3 han  Lfarmhh Lnonagbushh  dagepa Lfamilysize hasbro ftree10 grave10 i.region "  // strong predictor of marriage transition 
 
global iv "edu_fm i.children## lassetp10 Leduy Llincome  sp1 sp2 sp3 dagepa han  Llivepa Lfarmhh Lnonagbushh  hasbro Lfamilysize Lmigrant i.region "

logit livepa $iv if male==1 &  urbanhukou10==0
eststo  male_rural_children

logit livepa $iv if male==0 &  urbanhukou10==0
eststo  female_rural_children


logit livepa $iv if male==1 &  urbanhukou10==1
eststo  male_urban_children

logit livepa $iv if male==0 &  urbanhukou10==1
eststo  female_urban_children

esttab male_rural_children female_rural_children male_urban_children female_urban_children using  "$tables\children.rtf" ,    ///
       nonumbers mtitles  eform  ci(%9.2f) noomitted la replace 
